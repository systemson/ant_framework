    ...
    log(msg, level, date) {
        return new Promise((resolve) => {
            switch (level) {
                ...
                case "fatal":
                case "warn":
                    console.error(this.LOG_COLORS[level], `[${date}] | ${level.toUpperCase().padEnd(5, " ")} | ${msg}`, "\x1b[0m");
                    break;
                default:
                    console.log(this.LOG_COLORS[level], `[${date}] | ${level.toUpperCase().padEnd(5, " ")} | ${msg}`, "\x1b[0m");
                    break;
            }
            resolve();
        });
...
class FileLogger {
    constructor(folder, name, maxTicks = 10) {
        this.folder = folder;
        this.name = name;
    ...
    get fileName() {
        return `${this.name.toLowerCase()}-${(0, helpers_1.today)()}.log`;
    }
    getFileName(time) {
    ...
    init() {
        ...
        const maxDays = (0, helpers_1.getEnv)("APP_LOG_MAX_DAYS", "false");
        if (maxDays != "false") {
            fs_1.default.readdirSync(this.folder)
                .map(file => file.replace(`${this.name}-`, "").replace(".log", ""))
                .filter(file => file <= (0, moment_1.default)().subtract(parseInt((0, helpers_1.getEnv)("APP_LOG_MAX_DAYS", "10")), "days").format(helpers_1.DATE_FORMAT))
                .forEach(file => {
                fs_1.default.unlinkSync(`${this.folder}/${this.getFileName(file)}`);
            });
        }
        const maxSize = (0, helpers_1.getEnv)("APP_LOG_MAX_SIZE", "false");
        if (maxSize != "false") {
            ...
            if (fileSize >= parseInt(maxSize)) {
                fs_1.default.renameSync(fileName, `${this.folder}/${this.getFileName((0, moment_1.default)().unix().toString())}`);
            }
        }
    }
    clear() {
...
class Logger {
    static log(level, raw) {
        let msg;
        if (typeof raw != "string") {
            if (helpers_1.NODE_ENV == "development") {
