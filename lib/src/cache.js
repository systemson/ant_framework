...
class FilesystemChacheDriver {
    ...
    get(key, def) {
        return new Promise((resolve) => {
            resolve(this.decode(this.getFileData(key))?.data || def);
        });
    }
    unset(key) {
        return new Promise((resolve) => {
...
class RedisChacheDriver {
        ...
        return new Promise((resolve, reject) => {
            if (this.client === undefined) {
                ...
                this.client.on("connect", () => {
                    logger_1.Logger.info(helpers_1.Lang.__("Connected to redis server on [{{host}}:{{port}}].", {
                        host: this.config.host,
                        port: this.config.port.toString(),
                    }));
                    resolve();
    ...
    getRealKey(key) {
        return `${(0, StringUtils_1.snakeCase)((0, helpers_1.getEnv)("REDIS_CACHE_PREFIX"))}${key}`;
    }
}
